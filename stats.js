...
function main() {
  ...
  for (let i = 1; i < ordered.length; i++) {
    const seconds =
      (Date.parse(ordered[i].appliedAt) - Date.parse(ordered[i - 1].appliedAt)) / 1000;
  ...
  if (gaps.length) {
    ...
    console.log(
      `  fastest ${Math.round(gaps[0])}s   p25 ${at(0.25)}s   median ${at(0.5)}s   p90 ${at(0.9)}s`
    );
  ...
  for (const { name, count } of companies.slice(0, 8)) {
    ...
    console.log(
      `  ${String(count).padStart(3)} ${bar(count, heaviest, 18)} ${name.slice(0, 34)}${flag}`
    );
  }
  ...
  if (failed.length) {
    ...
    if (wasted > 0) {
      console.log(
        `  ${wasted} of those (${pct(wasted, failed.length)}%) were attempts beyond the ${cap}-try cap —`
      );
  ...
  for (const e of [...applied, ...failed]) {
    ...
    if (!day) continue;
    if (!runDays.has(day))
      runDays.set(day, { applied: 0, failing: new Set(), companies: new Set() });
    ...
    if (recent.length && earlier.length) {
      ...
      if (drop >= 15) {
        ...
        if (recentSpread > earlierSpread * 1.5) {
          console.log(
            `  Failures are spread across ~${Math.round(recentSpread)} employers per run, up from ~${Math.round(earlierSpread)}.`
          );
  ...
  if (coded.length) {
    const FAMILIES = [
      {
        ...
        advice:
          'The form was not where the bot expected. Selectors need updating — run --dry-run and watch the browser.',
      },
      {
        name: 'LinkedIn rejecting submissions',
        codes: ['unconfirmed_submit'],
        advice:
          'Submits are going through but not confirming. Often a sign the account is being throttled — slow down before it becomes a pause.',
      },
      {
        name: 'Your answers are incomplete',
        codes: ['unanswerable', 'invalid_field'],
        advice:
          'The bot had no answer, or one the form refused. needs-review.md names each question and the config key that fixes it.',
      },
      {
        name: 'Form or timing trouble',
        codes: ['stuck_form', 'timeout', 'error'],
        advice:
          'A step would not advance. Usually transient; persistent means the form shape changed.',
      },
    ];
    ...
    for (const family of scored) {
      console.log(
        `  ${String(family.count).padStart(4)}  ${family.name}  (${pct(family.count, coded.length)}%)`
      );
    }
    if (scored.length) {
  ...
  console.log(
    `  ${offTarget.length} of ${applied.length} (${pct(offTarget.length, applied.length)}%) would now be screened out`
  );
  const reasons = {};
  ...
  for (const e of applied.filter((x) => !assessTitle(x.title).skip)) {
    for (const word of cleanText(e.title)
      .toLowerCase()
      .replace(/[^a-z0-9.+#/ -]/g, ' ')
      .split(/[\s/-]+/)) {
