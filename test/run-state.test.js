...
test('the cap trips only after the configured number of applications', () => {
  ...
  try {
    ...
    assert.equal(companyCapReached('Acme Pvt Ltd'), false);
    noteCompanyApplication('Acme Private Limited');
...
test('a lifetime cap stops slow accumulation at one employer', () => {
  ...
  try {
    ...
    const rows = Array.from({ length: 3 }, (_, i) => ({
      jobId: String(i),
      company: i === 0 ? 'Acme Pvt Ltd' : 'Acme Private Limited',
      platform: 'LinkedIn',
      status: 'applied',
    ...
    assert.equal(
      logger.companyApplicationCount('Acme', 'LinkedIn'),
      3,
      'spellings share a counter'
    );
