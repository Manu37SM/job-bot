...
test('"Immediate" is recognised as zero — it carries no digits', () => {
  assert.equal(
    matchNumericOption('0', ['Immediate', '15 days', '30 days', '60 days']),
    'Immediate'
  );
  assert.equal(
...
test('"More than N" is an open range, not the point N', () => {
  ...
  assert.equal(
    matchNumericOption('10', ['At least 5 years', 'Less than 5 years']),
    'At least 5 years'
  );
...
test('salaryUnit orders the candidates for an ambiguous salary field', () => {
  ...
  try {
    ...
    assert.equal(
      buildNumericCandidates('7', 'Expected CTC', { inputType: 'number', step: '' })[0],
      '7'
    );
    config.salaryUnit = 'rupees';
    assert.deepEqual(
      buildNumericCandidates('7', 'Expected CTC', { inputType: 'number', step: '' }),
      ['700000']
    );
...
test('an explicit unit in the label always wins over the preference', () => {
  const { buildNumericCandidates } = require('../field-value');
  assert.equal(
    buildNumericCandidates('7', 'Expected CTC in LPA', { inputType: 'number', step: '' })[0],
    '7'
  );
  assert.deepEqual(
    buildNumericCandidates('7', 'Expected annual salary in INR', { inputType: 'number', step: '' }),
    ['700000']
  );
});
...
test('a field with a small max rules out the rupee reading', () => {
  const { buildNumericCandidates } = require('../field-value');
  const candidates = buildNumericCandidates('7', 'Expected CTC', {
    inputType: 'number',
    step: '',
    max: '100',
  });
...
test('a word answer survives a numeric-sounding label', () => {
  ...
  const looksNumeric = (answer) =>
    /^-?\d+(?:\.\d+)?$/.test(String(answer).trim().replace(/[,\s]/g, ''));
